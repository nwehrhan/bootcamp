...
impl JiraDatabase {
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        let mut current_db = self.read_db()?;

        let epic = &mut current_db
            .epics
            .get_mut(&epic_id)
            .ok_or_else(|| anyhow!("could not find epic in database!"))?;

        for story_id in &epic.stories {
            current_db.stories.remove(&story_id);
        }

        current_db.epics.remove(&epic_id);

        self.database.write_db(&current_db)?;
